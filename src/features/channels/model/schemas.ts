...
export const createChannelSchema = z.object({
  title: z.string().trim().min(2, "Название канала должно быть не короче 2 символов").max(80, "Название канала слишком длинное"),
  username: usernameSchema,
  description: z.string().trim().max(280, "Описание канала слишком длинное").optional().or(z.literal("")),
})
...
export const sendChannelMessageSchema = z.object({
  content: z.string().trim().min(1, "Сообщение не может быть пустым").max(1000, "Сообщение слишком длинное"),
})
